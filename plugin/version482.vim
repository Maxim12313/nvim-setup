...
autocmd BufWritePre * call version482#TextChanged()
...
function version482#initVersionDir(dirname)
	...
	if v:shell_error || stridx(l:out, 'eecs482') < 0
        ...
        :echo "version482 activated"
	    let l:out1 = systemlist('cd "' . a:dirname . '" ; git rev-parse --show-toplevel')[0]
	    if v:shell_error
		let s:versionDir[a:dirname] = ''
	    else
		let s:versionDir[a:dirname] = fnamemodify(l:out1, ':p') . '.version482'
